#!/usr/bin/env bash
# Launch online roleplaying game 'Abandoned Realms' using TinTin++ MUD client.

# See here for updates:
# http://sf.net/projects/artt/files/latest/download
...
# Create desktop shortcut (1 for yes, 0 for no)
desktop_shortcut=1
...
if [ ! -e "$desktop_file" ]; then
    ...
    # For mate-terminal, execute:
        ...
        echo "Exec=mate-terminal --window-with-profile=abandonedrealms --hide-menubar --title=AbandonedRealms --working-directory=$HOME/ARTT/ --maximize --command='tt++ $HOME/ARTT/main.tin'" >> $desktop_file
...
if [ "$terminal" = "mate-terminal" ] && [ "$desktop" = "mate" ]; then
    ...
    if [ -z "${PROFILES}" ]; then
        ...
        if [[ "${PROFILES}" =~ 'abandonedrealms' ]]; then
	        ...
	        echo "Entering Abandoned Realms..."
            mate-terminal --command="tt++ $HOME/ARTT/main.tin" --window-with-profile=abandonedrealms --maximize --hide-menubar --title=AbandonedRealms --working-directory=$HOME/ARTT/
            exit 1
